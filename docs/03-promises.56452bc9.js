...
refs.formEl.addEventListener('submit', function (e) {
  e.preventDefault();
  var step = refs.inputStepEl.value;
  countPromises(+step);
});
function countPromises(step) {
  var amount = refs.inputAmountEl.value;
  var delay = refs.inputDelayEl.value;
  var time = +delay;
  timerId = setInterval(function () {
    ...
    time += step;
    createPromise(promiseCounter + 1, time).then(function (x) {
...
function createPromise(position, delay) {
  return new Promise(function (resolve, reject) {
    ...
    setTimeout(function () {
      if (shouldResolve) {
        refs.outputPromises.insertAdjacentHTML('beforeend', "<p>\u2705 Fulfilled promise ".concat(position, " in ").concat(delay, "ms</p>"));
        return resolve({
          position: position,
          delay: delay
        });
      } else {
        refs.outputPromises.insertAdjacentHTML('beforeend', "<p>\u274C Rejected promise ".concat(position, " in ").concat(delay, "ms</p>"));
        return reject({
          position: position,
          delay: delay
        });
